...
def main():
    ...
    common_count = 0
    for map_path in library:
        count = 0
        lib_spectrum = Spectrum(f"{library_path}/{map_path}") #Создаем экземпяр класса Spectrum
        ...
        for spectrum in smoothed_spectra:
            trimmed_x, trimmed_y = trim(spectrum[:, 0], spectrum[:, 1], lib_spectrum.x) #Обрезаем спеткр до диапазона библиотечного
